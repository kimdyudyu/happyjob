...
//페이징 설정 
var refPageSize = 10;    	// 화면에 뿌릴 데이터 수 
var refPageSizevue = 10;
var refPageBlock = 10;		// 블럭으로 잡히는 페이징처리 수
...
function selectListvue(currentPage){
	...
	callAjax("/hls2/refList.do","post","json", true, param, resultCallback);
...
function gridinit(data,currentPage){
   ...
   var pagingnavi = getPaginationHtml(currentPage, totalCnt, refPageSizevue,refPageBlock, 'selectListvue');
...
// 	gfModalPop("#popvue");
	 ...
	 callAjax("/hls/detailRefList.do", "post", "json", true, param, resultCallback2);
}
...
function fdetailResult(data){
     ...
     var jsonstr = json_obj.detailRef;
     //console.log("jsonstr : " + json_obj);
	
	
     //console.log("fdetailResult data : " + data);
     //console.log("fdetailResult data : " + data.resultMsg);
	 //alert("공지사항 상세 조회  33");
	 if(data.resultMsg == "SUCCESS"){
		 //모달 띄우기 
		 gfModalPop("#popvue");
...
function frealPopModal(object){
	 ...
	 if(object == "" || object == null || object == undefined){
		 ...
		 //title, plan, no, loginID
		 //popvue.no_vue = object.plan;
		 popvue.title_vue = object.title;
		 popvue.start_vue = object.write_date;
		 popvue.end_vue = object.enddate;
		 popvue.filename_vue = object.filename;
		 popvue.plan_vue = object.plan;
		 //popvue.start_vue = object.loginID;
		 
		 
		 //popvue.no = object.no;
			
		 //console.log("AAAAAAAAAAAAAAA : " + object.no);
										...
										<table id="vuedatatable" class="col2 mb10" style="width: 1000px;">
											...
											<thead>
												<tr>
													...
													<th data-field="write_date">작성일</th>
					...
					<table class="row" id="detailtable">
						...
						<tbody>
							...
							<tr>
								...
								<th><button type="button" id="downloadBtn" >다운로드</button>	
							</tr>
							<tr align="center">
